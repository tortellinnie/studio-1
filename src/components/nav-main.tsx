                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton 
                    ...
                    className={cn(
                      ...
                      pathname === item.url 
                        ? "bg-white text-[#003da5] font-black shadow-xl" 
                        : "text-white/60 hover:bg-white/5 hover:text-white"
                    )}
                  >
                    <Link href={item.url}>
                      <item.icon className={cn("h-6 w-6 mr-5", pathname === item.url ? "text-[#003da5]" : "text-white/30 group-hover:text-white")} />
                      <span className="text-lg">{item.title}</span>
                      {pathname === item.url && <ChevronRight className="ml-auto h-5 w-5 text-[#003da5]/30" />}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
