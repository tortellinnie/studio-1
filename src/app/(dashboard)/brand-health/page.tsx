  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 pb-20">
      ...
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        <h1 className="text-5xl font-extrabold text-slate-900 tracking-normal">Hero SKU Podium</h1>
        <p className="text-lg text-slate-500 font-medium leading-relaxed tracking-normal">
          Strategic promo prioritization based on positive sentiment win-rate. Top performing P&G products computed from real-time NLP inference.
        </p>
      </div>

      <div className="max-w-5xl mx-auto w-full">
        <div className="bg-white border border-slate-200 rounded-[3rem] p-12 shadow-sm space-y-16">
          
          {/* Visual Podium Section */}
          <div className="flex items-end justify-center px-4 pt-24 h-80 relative max-w-4xl mx-auto">
            {podium.map((sku, index) => (
              <div key={sku.name} className={cn(
                ...
                <div className="absolute -top-32 flex flex-col items-center gap-3 w-full">
                  ...
                  <span className={cn(
                    "font-bold text-center px-4 transition-colors tracking-normal",
                    index === 1 ? "text-2xl text-slate-900" : "text-sm text-slate-500"
                  )}>
                    {sku.name}
                  </span>
                </div>
                ...
                <div className={cn(
                  ...
                  index === 1 ? "h-56 bg-[#003da5] shadow-lg shadow-blue-900/20" : // Rank 1
          ...
          <div className="flex justify-center pt-4">
            <button className="px-12 py-5 rounded-[2rem] bg-[#003da5] text-white font-bold text-sm uppercase tracking-normal hover:bg-blue-800 transition-all shadow-xl shadow-blue-900/20 active:scale-95 flex items-center gap-3">
              <Trophy className="h-5 w-5" /> Generate Performance Audit
            </button>
          </div>
        </div>
      </div>
