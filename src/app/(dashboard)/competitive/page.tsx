    ...
    <div className="space-y-8 animate-in fade-in duration-200 pb-20">
        ...
        {[
          ...
          <Card key={i} className={cn("chart-card border-l-4", item.color)}>
            <CardContent className="pt-6">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">{item.title}</h4>
              <div className="text-4xl font-bold text-slate-900 tabular-nums">{item.value}</div>
              <p className="text-xs text-muted-foreground mt-2 font-medium uppercase tracking-tight">{item.sub}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card className="chart-card overflow-hidden">
        <CardHeader className="py-6 px-8 border-b border-slate-100 bg-slate-50/30">
          <CardTitle className="text-lg font-bold text-slate-900">Market Positioning Matrix (Share vs Sentiment)</CardTitle>
        </CardHeader>
          ...
          <div className="h-[450px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
                ...
                <XAxis type="number" dataKey="sentiment" name="Sentiment" unit="%" stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} tick={{ fontWeight: 500 }} />
                <YAxis type="number" dataKey="marketShare" name="Share" unit="%" stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} tick={{ fontWeight: 500 }} />
                <ZAxis type="number" dataKey="growth" range={[100, 1000]} />
                <Tooltip 
                  cursor={{ strokeDasharray: '3 3' }}
                  contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', fontWeight: 600, fontSize: '12px' }}
                />
                <Legend verticalAlign="top" align="right" height={40} iconType="circle" wrapperStyle={{ fontWeight: 600, fontSize: '12px' }} />
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50 text-[11px] uppercase font-bold text-slate-400 tracking-widest">
                    ...
                    <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="p-6">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-md font-bold text-slate-900 group-hover:text-[#003da5] transition-colors">{item.name}</span>
                      ...
                      <td className="p-6 text-center">
                        <div className="flex items-center justify-center gap-1.5 font-bold text-[#003da5] text-2xl tabular-nums">
