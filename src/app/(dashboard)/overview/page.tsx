  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-16">
      <div className="space-y-2">
        ...
        <p className="text-lg font-medium text-slate-500">Real-time P&G e-commerce performance analytics</p>
      </div>
        ...
        {[
          ...
          <Card key={i} className="border-border/50 shadow-md bg-white">
            <CardHeader className="pb-3">
              <CardTitle className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">{item.title}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className={cn("text-4xl font-black", item.color)}>{item.value}</div>
              <p className="text-xs text-slate-500 font-bold mt-3 uppercase tracking-tight">{item.sub}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 border-border/50 shadow-md bg-white">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-slate-800">Historical Sentiment Trends</CardTitle>
            <CardDescription className="text-sm">Monthly performance tracking vs key competitors</CardDescription>
          </CardHeader>
          <CardContent className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={multiBrandTrends}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <XAxis dataKey="month" stroke="#94a3b8" fontSize={14} tickLine={false} axisLine={false} dy={10} />
                <YAxis stroke="#94a3b8" fontSize={14} tickLine={false} axisLine={false} domain={[50, 80]} />
                <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 8px 16px rgba(0,0,0,0.1)' }} />
                <Legend iconType="circle" wrapperStyle={{ paddingTop: '30px', fontSize: '14px', fontWeight: 600 }} />
                <Line type="monotone" dataKey="P&G" stroke="#003da5" strokeWidth={5} dot={{ r: 6, fill: '#003da5' }} activeDot={{ r: 9 }} />
                <Line type="monotone" dataKey="Surf" stroke="#94a3b8" strokeWidth={2} strokeDasharray="6 6" />
                <Line type="monotone" dataKey="Breeze" stroke="#cbd5e1" strokeWidth={2} strokeDasharray="6 6" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50 shadow-md bg-white">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-slate-800">Sentiment Distribution</CardTitle>
            <CardDescription className="text-sm">Overall portfolio health</CardDescription>
          </CardHeader>
            ...
            <div className="w-full mt-10 grid grid-cols-3 gap-4">
              {sentimentDist.map((item) => (
                <div key={item.name} className="flex flex-col items-center">
                  <span className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">{item.name}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="border-border/50 shadow-md bg-white">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-slate-800">5 Vectors of Superiority</CardTitle>
            <CardDescription className="text-sm">Benchmarking vs competitor average</CardDescription>
          </CardHeader>
          <CardContent className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={vectorAvg} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                ...
                <Legend iconType="rect" wrapperStyle={{ paddingTop: '30px', fontSize: '14px', fontWeight: 600 }} />
        ...
        <Card className="border-border/50 shadow-md bg-white">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-slate-800">Top Performing SKUs</CardTitle>
            <CardDescription className="text-sm">Ranked by corrected rating</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-border/50">
              {pngProducts.slice(0, 6).map((product) => (
                <div key={product.id} className="flex items-center justify-between p-6 hover:bg-slate-50 transition-colors">
                  <div className="space-y-1">
                    <p className="text-lg font-bold text-slate-900">{product.name}</p>
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest">{product.reviewCount.toLocaleString()} verified reviews</p>
                  </div>
                  <div className="flex items-center gap-8">
                    <div className="text-right">
                      <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Original</p>
                      <p className="text-base font-bold text-slate-400 font-mono">{product.originalRating}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Corrected</p>
                      <p className="text-xl font-black text-emerald-600 font-mono">{product.correctedRating}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Card className="border-border/50 shadow-lg bg-slate-50/50">
        <CardHeader className="pb-6">
          <CardTitle className="text-xs font-black text-slate-400 uppercase tracking-[0.4em]">Strategic Intelligence Recap</CardTitle>
        </CardHeader>
        <CardContent className="grid md:grid-cols-2 gap-8">
          {[
            ...
            <div key={i} className="flex items-start gap-6 p-6 bg-white rounded-2xl shadow-sm border border-slate-100">
              <insight.icon className={cn("h-7 w-7 shrink-0 mt-1", insight.color)} />
              <div className="space-y-2">
